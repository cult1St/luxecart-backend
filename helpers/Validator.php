...
class Validator
{
    ...
    protected $_factoryValidationErrorMsgs = [
        ...
        'inexistent_validation' => 'Validation filter {filter} does not exist for {field}. Contact admin.',
    ...
    protected $_basicHTMLTags = '<blockquote><br><code><dd><dl>'
    ...
    {
        if (
            !isset($this->_filters[$field]['field'])
        ...
        if (
            !isset($this->_filters[$field]['label'])
        ...
        if (!isset($this->_filters[$field]['labelLineageFwd'])) {
            ...
            $this->_filters[$field]['labelLineageFwd'] =
                implode(
                    $this->_fieldLabelFwdHierarchyDelimiter,
                    $labelLineage
                );
            $this->_filters[$field]['labelLineageRev'] =
                implode(
                    $this->_fieldLabelRevHierarchyDelimiter,
     ...
     * @throws Exception if preg_match_all fails
     */
    private function _convertFiltersStringToArray(
        ...
        foreach ($filters as $filter) {
            if (
                ...
                if ($isValidation) {
                    $addFilter['errormsg'] =
                        isset($filter['errormsg_b']) ? $filter['errormsg_b'] : (isset($filter['errormsg_a']) ? $filter['errormsg_a'] : '');
                }
     ...
     */
    private function _readableErrorMsg(
        string $errorMsg,
        string $field,
        $value = '',
        string $filter = '',
        ...
        $errorMsg = str_ireplace(
            ['{fieldlineage}', '{labellineage}'],
            $spanTags['field'][0] . $this->_filters[$field]['labelLineageRev']
                . $spanTags['field'][1],
            $errorMsg
        );
        $errorMsg = str_ireplace(
            ['{fieldlineagef}', '{labellineagef}'],
     ...
     * @return object
     */
    public function setHTMLSpansForErrorMsgs(
        string $errormsgSpan = NULL,
        string $fieldSpan = NULL,
        string $valueSpan = NULL,
        string $filterSpan = NULL,
        string $argSpan = NULL
    ) {
        foreach (['errormsg', 'field', 'value', 'filter', 'arg'] as $element) {
     ...
     * @return object
     */
    public function setSanitizations(
        array $sanitizations,
                    ...
                    switch (TRUE) {
                        case (isset($this->_customSanitizations[$filter])):
                            $fieldValue = call_user_func(
                                $this->_customSanitizations[$filter],
                                $fieldValue,
                                $argsSynonyms
                            );
                            break;
    ...
    public function runValidations($input)
    {
        ...
        foreach ($this->_filters as $field => $fieldFilters) {
            ...
            foreach ($fieldFilters['validations'] as $fieldFilter) {
                ...
                if (
                    in_array($method = $filter, ['required', 'notempty'])
